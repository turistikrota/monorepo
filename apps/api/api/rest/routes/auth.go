...
func Auth(router fiber.Router, srv restsrv.Srv, app app.App) {
	...
	group.Post("/login/start", srv.VerifyTokenExcluded(), srv.Turnstile(), srv.Timeout(authLoginStart(srv, app)))
	group.Post("/login/verify", srv.AccessInit(), srv.AccessExcluded(), srv.VerifyTokenRequired(), srv.Timeout(authLoginVerify(srv, app)))
...
func authLoginVerify(srv restsrv.Srv, app app.App) fiber.Handler {
	return func(c *fiber.Ctx) error {
		var cmd commands.AuthLoginVerify
		if err := c.BodyParser(&cmd); err != nil {
			return err
		}
		cmd.Device = srv.MakeDevice(c)
		cmd.VerifyToken = middlewares.VerifyTokenParse(c)
		res, err := app.Commands.AuthLoginVerify(c.UserContext(), cmd)
		if err != nil {
			return err
		}
...
func authLoginStart(srv restsrv.Srv, app app.App) fiber.Handler {
	return func(c *fiber.Ctx) error {
		var cmd commands.AuthLoginStart
		if err := c.BodyParser(&cmd); err != nil {
			return err
		}
		cmd.Device = srv.MakeDevice(c)
		res, err := app.Commands.AuthLoginStart(c.UserContext(), cmd)
		if err != nil {
			return err
		}
